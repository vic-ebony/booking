...
def update_revision_and_version(doc_path, new_revision_date, new_revision_version):
    """
    更新 Word 文件的頁首中包含 "修訂日期" 和 "版本" 的欄位。
    """
    word_app = win32.gencache.EnsureDispatch("Word.Application")
    word_app.Visible = False
    ...
    try:
        ...
        for section in doc.Sections:
            header_range = section.Headers(win32.constants.wdHeaderFooterPrimary).Range
            header_text = header_range.Text
            if "修訂日期：" in header_text:
                header_range.Text = header_text.replace(
                    header_text.split("修訂日期：")[-1].split(" ")[0],
                    new_revision_date
                )
            if "版本：" in header_text:
                header_range.Text = header_text.replace(
                    header_text.split("版本：")[-1].split("\r")[0],
                    new_revision_version
                )
        doc.Save()
        print(f"已成功更新 Word 文件中的修訂日期和版本: {doc_path}")
    except Exception as e:
        print(f"更新 Word 文件中的修訂日期和版本時發生錯誤: {doc_path}, 錯誤訊息: {e}")
    finally:
        doc.Close(False)
        word_app.Quit()

def update_word_table(doc_path, new_date, new_version):
    """
    更新 Word 文件中第一頁表格中包含 "頒佈日期" 和 "版本" 的欄位的日期和版本。
    """
    word_app = win32.gencache.EnsureDispatch("Word.Application")
    word_app.Visible = False
    ...
    try:
        ...
        for row in table.Rows:
            ...
            elif "版本" in cell_text:
                row.Cells(2).Range.Text = new_version
        doc.Save()
        print(f"已成功更新 Word 文件中的表格日期和版本: {doc_path}")
    except Exception as e:
        print(f"更新 Word 文件中的表格日期和版本時發生錯誤: {doc_path}, 錯誤訊息: {e}")
    finally:
        doc.Close(False)
        word_app.Quit()

def batch_process_folder(folder_path, replacements, target_height, target_width, tolerance=0.15, 
                         new_date=None, new_version=None, new_revision_date=None, new_revision_version=None):
    """
    ...
    for root, _, files in os.walk(folder_path):
        for file in files:
            ...
            if file.endswith(".docx") or file.endswith(".doc"):
                ...
                if new_date or new_version:
                    update_word_table(file_path, new_date, new_version)
                if new_revision_date or new_revision_version:
                    update_revision_and_version(file_path, new_revision_date, new_revision_version)
...
if __name__ == "__main__":
    ...
    new_version = "3.33"  # 新的版本
    new_revision_date = "2030.01.01"  # 新的修訂日期
    new_revision_version = "5.55"  # 新的修訂版本

    if os.path.exists(folder_path):
        batch_process_folder(folder_path, replacements, target_height, target_width, tolerance, 
                             new_date, new_version, new_revision_date, new_revision_version)
    else:
